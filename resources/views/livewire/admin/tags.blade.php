<div>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        @if (session()->has('success'))
            <a class="flex items-center justify-between p-4 mb-8 text-sm font-semibold text-purple-100 bg-purple-600 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
                ...
                <span>Attach Products &RightArrow;</span>
            </a>
        @endif
        <h2 class="mt-4 ml-4 text-xs font-semibold text-gray-700 dark:text-gray-200">
            Add Tag
        </h2>
        <form wire:submit.prevent="storeTag" class="flex justify-around ml-4 mt-2 flex-1 lg:mr-32">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <flux:modal.trigger name="edit-tag">
                                        <button
                                            wire:click="$dispatch('open-tag-modal', { mode: 'edit', tag: '{{ $tag->slug }}' })"
                                    ...
                                    <flux:modal.trigger name="delete-tag">
                                        <button wire:click="$dispatch('delete-tag', {id: {{ $tag->id}}})"
                                            class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                            aria-label="Delete">
                                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
    ...
    <flux:modal name="edit-tag" class="w-full bg-white rounded-t-lg dark:bg-gray-800  sm:rounded-lg sm:max-w-xl">
        <div>
            <div class="text-center mt-8 mb-2">
                <flux:text class="text-gray-700 dark:text-gray-400">Edit Tag
                </flux:text>
            </div>

            <div class="mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <form method="POST" wire:submit.prevent="updateTag" id="update_tag">
                    @csrf
            ...
            <div class="flex">
                ...
                <button type="submit" form="update_tag"
    ...
    <flux:modal name="delete-tag" class="bg-white rounded-t-lg dark:bg-gray-800">
        <div>
            <div class="text-center mt-8 mb-2">
                <flux:text class="text-gray-700 dark:text-gray-400">Delete Tag?
                </flux:text>
            </div>

            <div class="mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <span class="text-sm text-gray-600 dark:text-gray-400">You're about to delete this tag</span>
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    This will affect product under this tag</p>
            </div>
