<div>
    ...
    <div class="grid gap-6 mb-8 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="flex items-center p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
            ...
            <div>
                ...
                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                    {{ Number::currency($totalRevenue, 'NGN') }}
                </p>
            </div>
        </div>
        <!-- Card -->
                    ...
                    @forelse($recentOrders as $recentOrder)
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-sm">
                                {{ Number::currency($recentOrder->total_price, 'NGN') }}
                            </td>
    ...
    <script>
        setTimeout(() => {
            // Assuming your PHP data is being passed correctly
            const lineGraphData = @json($lineGraphData);

            // Format the revenue values to 2 decimal places
            const formattedData = lineGraphData.map((item) => ({
                ...item,
                revenue: item.revenue.toFixed(2), // Format revenue to 2 decimal places
            }));

            const lineConfig = {
                type: 'line',
                data: {
                    labels: formattedData.map((item) => item.day), // Use 'day' instead of 'month'
                    datasets: [
                        {
                            label: 'Daily Sales',
                            backgroundColor: '#0694a2',
                            borderColor: '#0694a2',
                            data: formattedData.map((item) => parseFloat(item.revenue)), // Ensure revenue is a float
                            fill: false,
                        },
                    ],
                },
                options: {
                    ...
                    tooltips: {
                        ...
                        callbacks: {
                            label: function (tooltipItem) {
                                // Format tooltip labels with two decimal places
                    ...
                    scales: {
                        x: {
                            ...
                            scaleLabel: {
                                display: true,
                                labelString: 'Day of Week', // Change 'Month' to 'Day of Week'
                            },
                        },
                        y: {
                            display: true,
                            ...
                            ticks: {
                                ...
                                callback: function (value) {
                                    // Format Y axis ticks with two decimal places
