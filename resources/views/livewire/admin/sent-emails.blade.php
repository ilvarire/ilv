<div x-data="{ showModal: false, action: '', emailId: null }" x-on:modal-close.window="showModal = false">
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        @if (session()->has('success'))
            <a class="flex items-center justify-between p-4 mb-8 text-sm font-semibold text-purple-100 bg-purple-600 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
                ...
                <span>Send email &RightArrow;</span>
            </a>
        @endif
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <button wire:click="edit({{ $email->id }})"
                                        @click="action = 'edit'; emailId = {{ $email->id }}; showModal = true"
                                    ...
                                    <button
                                        @click="action = 'delete'; $wire.set('deleteId', {{ $email->id }}); emailId = {{ $email->id }}; showModal = true"
                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                        aria-label="Delete">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div x-show="showModal" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0"
        ...
        <div x-show="showModal" x-transition:enter="transition ease-out duration-150" x-cloak
            ...
            class="w-full px-6 py-6 bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl"
                    ...
                    <div>
                        ...
                        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                            ...
                            <label class="block text-sm mb-2">
                                ...
                                <input type="text" wire:model="viewAdmin"
                            ...
                            <label class="block text-sm mb-2">
                                ...
                                <textarea wire:model="viewMessage"
                            ...
                            <label class="block text-sm mb-2">
                                ...
                                <input type="text" wire:model="viewReceivers"
                                    class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                                    placeholder="" disabled />
                            </label>
                            <label class="block text-sm mb-2">
                                ...
                                <input type="text" wire:model="viewNumber"
                                    class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input"
                                    placeholder="" disabled />
                            </label>
                            ...
                            @if ($viewImage)
                                <div class="grid gap-6 mb-6 mt-4 md:grid-cols-2 xl:grid-cols-4">
                                    <img src="{{ asset('storage/' . $viewImage) }}" alt="img" class="rounded-lg">
                                </div>
                            @endif
                ...
                <template x-if="action === 'delete'">
                    <div>
                        ...
                        <p class="text-sm text-gray-700 dark:text-gray-400">
                            Are you sure you want to delete email?<br>
                            Click ok to confirm
                        </p>
                        ...
                        <footer
                            ...
                            <button @click="$wire.deleteEmail; showModal = false"
