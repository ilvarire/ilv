                        ...
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-sm">
                                <span
                                    ...
                                    @elseif ($order->status === 'processing') px-2 py-1 font-semibold leading-tight text-orange-700 bg-orange-100 rounded-full dark:text-white dark:bg-orange-600
                ...
                @if ($selectedOrder)
                            ...
                            <div class="mt-4 mb-6">
                                <div>
                                    ...
                                    <p class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
                                        Amount: <strong>
                                            {{ Number::currency($selectedOrder->total_price, 'NGN') }}
                                        </strong>
                                    </p>
                                    ...
                                    <p class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
                                        Disount:
                                        {{ $selectedOrder->coupon_id ? $selectedOrder->coupon->code . ' (' .
                    $selectedOrder->coupon->discount_percentage . '%off)' : 'N/A' }}
                                    </p>
                                    <p class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
                                        Status:
                                        <span
                                    ...
                                    <a href="{{ route('admin.payments', ['search' => $selectedOrder->reference]) }}">
                                        <p class="mb-2 mt-4 text-sm text-purple-600 px-4 py-2 font-semibold">
                                            Payment:
                                            <strong>{{ str($selectedOrder->payment->status)->title() }}</strong>
                                ...
                                @foreach ($selectedOrder->items as $item)
                                    <!-- Modal description -->
                                    <a href="{{ route('admin.products', ['product_id' => $item->product_id])}}">
                                        <div class="flex px-4 py-3 mb-8 mt-2 bg-white rounded-lg shadow-md dark:bg-gray-800">
                                            <div class="hidden mt-4 w-8 h-8 mr-3 rounded-full md:block">
                                                <img class="object-cover w-full h-full rounded-lg"
                                                    src="{{ asset('storage/' . $item->product->images->first()->image_url)}}" alt=""
                                                    loading="lazy" />
                                            </div>
                                            <p class="text-xs text-gray-600 dark:text-gray-400">
                                                {{ $item->product->name }} <br>
                                                Quantity: {{$item->quantity}} <br>
                                                Price: {{ Number::currency($item->product->price, 'NGN') }} =
                                                {{ Number::currency($item->product->price * $item->quantity, 'NGN') }}
                                            </p>
            ...
            <div class="flex">
                ...
                @if ($selectedOrder)
                    @if($selectedOrder->status === 'pending')
                        <button type="submit" wire:click="confirmOrderPayment('{{ $selectedOrder->id }}')"
                    ...
                    @elseif($selectedOrder->status === 'processing')
                        <button wire:click="shipOrder('{{ $selectedOrder->id }}')"
                            wire:loading.class="opacity-50 cursor-not-allowed"
                            wire:loading.remove.class="active:bg-purple-600 hover:bg-purple-700 focus:shadow-outline-purple"
