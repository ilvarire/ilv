<div>
                                    ...
                                    <div>
                                        <p class="font-semibold"> {{$product->name}}
                                            @if ($product->is_featured)
                                                <span
                                                    class="inline-flex items-center rounded-md bg-red-100 px-2 py-0 text-xs font-medium text-red-700 inset-ring inset-ring-red-100">
                                                    Featured
                                                </span>
                                            @endif
                                            {{ $product->is_active ? '✅' : '❌' }}
                                        ...
                                        <p
                                            class='{{ $product->quantity <= 10 ? "text-xs text-red-700 dark:text-red-400" : "text-xs text-gray-600 dark:text-gray-400" }}'>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <flux:modal.trigger name="edit-product">
                                        <button
                                            wire:click="$dispatch('edit-product', { mode: 'edit', product: '{{ $product->slug }}'})"
                                    ...
                                    <flux:modal.trigger name="delete-product">
                                        <button wire:click="$dispatch('delete-product', { id: {{ $product->id }} })"
                                            class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                            aria-label="Delete">
                                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
    ...
    <flux:modal name="edit-product" class="w-full bg-white rounded-t-lg dark:bg-gray-800  sm:rounded-lg sm:max-w-xl">
        <div>
            <div class="text-center mt-8 mb-2">
                <flux:text class="text-gray-700 dark:text-gray-400">Edit product
                </flux:text>
            </div>

            <div class="mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <form method="POST" id="update_product" wire:submit.prevent="updateProduct">
    ...
    <flux:modal name="delete-product" class="bg-white rounded-t-lg dark:bg-gray-800">
        <div>
            <div class="text-center mt-8 mb-2">
                <flux:text class="text-gray-700 dark:text-gray-400">Delete product?
                </flux:text>
            </div>

            <div class="mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <span class="text-sm text-gray-600 dark:text-gray-400">You're about to delete this product</span>
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    This could affect transactions under this product</p>
            </div>
